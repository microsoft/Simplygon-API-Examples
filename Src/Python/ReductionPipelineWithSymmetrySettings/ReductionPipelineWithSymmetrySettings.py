...
def SaveScene(sg: Simplygon.ISimplygon, sgScene: Simplygon.spScene, path: str):
    ...
    outputScenePath = ''.join(['output\\', 'ReductionPipelineWithSymmetrySettings', '_', path])
...
def RunReduction(sg: Simplygon.ISimplygon):
    ...
    # Enable symmetry detection in the reduction process. 
    sgReductionSettings.SetKeepSymmetry( True )
    sgReductionSettings.SetSymmetryAxis( Simplygon.ESymmetryAxis_X )
    sgReductionSettings.SetSymmetryOffset( 0.0 )
    sgReductionSettings.SetSymmetryDetectionTolerance( 0.0001 )
