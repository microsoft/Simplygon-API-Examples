...
void SaveScene(Simplygon::ISimplygon* sg, Simplygon::spScene sgScene, const char* path)
{
	...
	std::string outputScenePath = std::string("output\\") + std::string("RemeshingPipelineWithMaxDeviation") + std::string("_") + std::string(path);
...
void RunRemeshing(Simplygon::ISimplygon* sg)
{
	...
	// Switch target to MaxDeviation from the default OnScreenSize. 
	sgRemeshingSettings->SetRemeshingTarget( Simplygon::ERemeshingTarget::MaxDeviation );
	
	// Set the MaxDeviation. 
	sgRemeshingSettings->SetMaxDeviation( 0.02f );
	
	// Set the max derived OnScreenSize, which limits the maximum allowed geometry generation 
	// resolution when using a MaxDeviation target. 
	sgRemeshingSettings->SetMaxDerivedOnScreenSize( 2000 );
